document.addEventListener('DOMContentLoaded', () => {
    ...
    // Controle do Cronômetro Global
    let globalTimer = null;
    let timeRemaining = 240; // 4 minutos
    ...
    function startGlobalTimer() {
        ...
        timerStarted = true;
        timeRemaining = 240;
        document.getElementById('global-timer-widget').classList.remove('hidden');
        document.getElementById('timer-countdown').innerText = '04:00';
    ...
    function updateGlobalTimer() {
        ...
        if (timeRemaining <= 0) {
            ...
            document.getElementById('gathe-timeout-modal').classList.remove('hidden');
            setTimeout(() => {
                location.reload();
            }, 4000);
        }
    ...
    document.getElementById('btn-refresh-timer').addEventListener('click', () => {
        const mins = Math.floor(timeRemaining / 60);
        const secs = timeRemaining % 60;
        document.getElementById('timer-countdown').innerText = `${String(mins).padStart(2, '0')}:${String(secs).padStart(2, '0')}`;
    });
    ...
    let showingGatheWarning = false;

    function resetGatheSpeech() {
        gatheTimerFinished = false;
        gatheSpeechFinished = false;
        showingGatheWarning = true;
        ...
        // Exibe a fala de aviso
        applyTypewriter(p, "Espere eu terminar de falar por favor!", 30, () => {
            // Espera 2 segundos com o aviso na tela antes de reiniciar a fala principal
            setTimeout(() => {
                showingGatheWarning = false;
                const originalText = p.getAttribute('data-original-text') || p.innerText;
                applyTypewriter(p, originalText, 104, () => {
    ...
    btnNext1.addEventListener('click', () => {
        ...
        if (nameVal.length < 3 || nameVal === 'SemNome') {
            alert("⚠️ O Nome deve ter pelo menos 3 caracteres e não pode ficar em branco!");
            return;
        }
    ...
    document.getElementById('btn-next-2').addEventListener('click', () => {
        // Validação estrita: o usuário precisa ter clicado para alterar a data padrão
        if (clickCount === 0 || year === 1900) {
            alert("⚠️ Insira uma data de nascimento válida! É obrigatório preencher este campo (clique nos botões de incremento).");
            return;
        }
        goToScreen('screen-3');
    });
    ...
    document.getElementById('btn-next-3').addEventListener('click', () => {
        // Validação estrita: o telefone não pode estar em branco ou não preenchido
        if (!phoneStr || phoneStr === 'DIGITE SEU NÚMERO...' || phoneStr.trim() === '') {
            alert("⚠️ Insira seu número de telefone em algarismos romanos! Não pode ficar em branco.");
            return;
        }
    ...
    document.getElementById('btn-next-5').addEventListener('click', () => {
        // Validação estrita: verificar se o canvas tem alguma coisa desenhada
        const imgData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        const data = imgData.data;
        let hasDrawing = false;
        for (let i = 0; i < data.length; i += 4) {
            if (data[i + 3] > 0) { // canal alpha maior que zero
                hasDrawing = true;
                break;
            }
        }
        if (!hasDrawing) {
            alert("⚠️ Por favor, desenhe seu retrato no quadro de pixel art! Não pode ser enviado em branco.");
            return;
        }
    ...
    document.getElementById('btn-next-6').addEventListener('click', () => {
        // Validação: todas as 5 perguntas respondidas
        ...
        if (!allAnswered) {
            alert("⚠️ Responda a todas as perguntas do questionário de aptidão antes de prosseguir!");
            return;
        }
    ...
    function shuffleUnmatchedCards() {
        ...
        const unmatchedTerms = unmatchedCards.map(c => c.dataset.term);
        unmatchedTerms.sort(() => Math.random() - 0.5);
        unmatchedCards.forEach((c, i) => {
            c.dataset.term = unmatchedTerms[i];
            c.innerText = unmatchedTerms[i];
    ...
    document.getElementById('btn-final-presentation').addEventListener('click', () => {
        goToScreen('screen-8-5');
    });

    // === TELA 8-5: ACEITE DO CONTRATO ===
    const contractScroll = document.getElementById('contract-scrollbox');
    ...
    contractScroll.addEventListener('scroll', () => {
        // Tolerância de 5 pixels para detectar rolagem completa até o fim
        if (contractScroll.scrollHeight - contractScroll.scrollTop - contractScroll.clientHeight < 5) {
            contractScrolledToBottom = true;
            validateContractAcceptance();
        }
    });

    contractChecks.forEach(chk => {
        chk.addEventListener('change', validateContractAcceptance);
    });

    function validateContractAcceptance() {
        const allChecked = Array.from(contractChecks).every(chk => chk.checked);
        if (allChecked && contractScrolledToBottom) {
            btnSignContract.disabled = false;
            btnSignContract.classList.remove('btn-dark-disabled');
        } else {
            btnSignContract.disabled = true;
            btnSignContract.classList.add('btn-dark-disabled');
        }
    }

    btnSignContract.addEventListener('click', () => {
        // Para o cronômetro global ao concluir e assinar o contrato
        if (globalTimer) {
            clearInterval(globalTimer);
        }
